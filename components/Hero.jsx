...
const Hero = () => {
  ...
  return (
    <section className="min-h-screen flex items-center justify-center px-4 py-20 relative overflow-hidden">
      ...
      <div className={`${styles.hero} relative z-10 max-w-7xl mx-auto w-full m-4 p-6 md:m-8 md:p-12`}>
            ...
            <div className="flex flex-wrap gap-3 md:gap-4">
              {socialLinks.map((link, index) => (
                <motion.a 
                  ...
                  className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-primary-fg hover:text-white transition-all duration-300 border border-white/10 hover:scale-110"
                  whileHover={{ y: -5 }}
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: index * 0.1 }}
                >
                  <FontAwesomeIcon icon={link.icon} className="text-sm md:text-lg" />
                </motion.a>
              ))}
              ...
              >
                <FontAwesomeIcon icon={faKey} className="text-xs md:text-sm" />
              </motion.a>
            </div>

            {/* Call to Action Buttons */}
            <motion.div 
              className="flex flex-col sm:flex-row gap-4 md:gap-6 items-center pt-4"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 1 }}
            >
              ...
              >
                <span className="relative z-10 flex items-center justify-center gap-2">
                  Download Resume
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 md:h-5 md:w-5 group-hover:translate-y-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
